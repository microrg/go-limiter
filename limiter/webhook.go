...
func SendWebhook(url string, token string, userID string, value int, limit int) error {
	logger.Infof("User %s, triggering webhook: %s", userID, url)

	var jsonStr = []byte(fmt.Sprintf(`{"user_id":%s,"value":%d,"limit":%d}`, userID, value, limit))
